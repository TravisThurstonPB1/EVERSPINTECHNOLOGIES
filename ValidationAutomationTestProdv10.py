...
#				manualSAPnonexist.append(reason)
				...
				# if testresult4 != None:
					...
					# importSAP.append(verify)
				# else:
				test9=("Select T2.ItemCode, isnull((Select IBT1.Batchnum from IBT1 where IBT1.Basetype = 59 and IBT1.BsDocEntry = T1.DocEntry and IBT1.BsDocType = 202 and IBT1.BatchNum = cast(T3.Notes as nvarchar)),T0.BatchNum) 'AssyLotNo', T0.BatchNum, cast(t0.Quantity as int) 'quantity', T0.WhsCode, cast(isnull((select sum(quantity) from (Select IBT1.Quantity from IBT1 with(nolock) where IBT1.Basetype = 59 and IBT1.BsDocEntry = T1.DocEntry and IBT1.BsDocType = 202 and IBT1.BatchNum = cast(T3.Notes as nvarchar) and Direction = 0)S0),0) - isnull((Select sum(quantity) from (Select IBT1.Quantity from IBT1 with(nolock) where IBT1.Basetype = 59 and IBT1.BsDocEntry = T1.DocEntry and IBT1.BsDocType = 202 and IBT1.BatchNum = cast(T3.Notes as nvarchar) and Direction = 1)S1),0) as int)'On hand' from EverspinTech.dbo.IBT1 T0 with(nolock) inner join EverspinTech.dbo.OWOR T1 with(nolock) on T0.BsDocEntry = T1.DocEntry and T0.BsDocType = 202 inner join EverspinTech.dbo.WOR1 T2 with(nolock) on T1.DocEntry = t2.DocEntry left join EverspinTech.dbo.OBTN T3 with(nolock) on T0.BatchNum = T3.DistNumber where t0.Batchnum = '{2}' and T1.type = 'D'".format(x[2],x[2],x[2]))
	...
	#					print('On hand is 0')
						...
						if testresult10[5] >= x[3]:
							verify = (x[0], x[1], x[2], x[3], testresult10[1], testresult10[3], testresult10[0], testresult10[4])
	#						print("Line 162", verify)
							importSAP.append(verify)
						else:
							reason = (x[0], x[1], x[2], x[3], "SAP On Hand Quantity for " + testresult10[0] + " less than planned Qty.  Cannot Create PRDO.  Line 164 query result.", '0')
							manualSAPnonexist.append(reason)
				else:
...
#####Verify If data should exist in SAP######	
	...
	try:
		...
		for x in insertSAPComp:
			...
			test = ("Select T0.Itemcode, T0.Batchnum, T0.Quantity, T0.BsDocEntry from IBT1 T0 With(nolock) where T0.BatchNum = '{0}' and T0.BsDocType = 202".format(g))
			cursor.execute(test)
			result = cursor.fetchone()
			if result != None:
				pass
